...
| Base processada | ✅ | Features por frame e por janela materializadas em parquet+CSV (`frame_features_real`, `window_features_real`) com relatório de qualidade versionado (`pipeline_version=fase4-v1`) | `data/processed/` | Dataset pronto para análise e modelagem (Fase 4, 2026-04-25) |
| Relatório de qualidade de dados | ✅ | Métricas de completude, ranges e linhagem da base processada | [data/processed/data_quality_report.json](../data/processed/data_quality_report.json) | Relatório gerado por `compute_quality_report` com `pipeline_version=fase4-v1` (Fase 4, 2026-04-25) |
| Linhagem versionada | ✅ | Colunas `source_split`, `frame_stride`, `window_size`, `pipeline_version`, `extraction_timestamp` em frames e janelas | `data/processed/frame_features_real.*`, `data/processed/window_features_real.*` | Linhagem presente em todas as linhas da base processada (Fase 4, 2026-04-25) |
...
| Feature engineering | ✅ | Consolidação de atributos analíticos por frame e por janela com schemas explícitos (`FRAME_FEATURES_SCHEMA`, `WINDOW_FEATURES_SCHEMA`) e linhagem versionada (`pipeline_version=fase4-v1`) | [src/mediapipe_seguranca/feature_engineering_real.py](../src/mediapipe_seguranca/feature_engineering_real.py), [src/mediapipe_seguranca/build_processed_base.py](../src/mediapipe_seguranca/build_processed_base.py) | Features úteis e documentadas, validadas por 6 testes em [tests/test_feature_engineering_real.py](../tests/test_feature_engineering_real.py) (Fase 4, 2026-04-25) |
| Pipeline orquestradora | ✅ | Fluxo `--mode demo`, `--mode real` e `--mode processed` (com `--window-size`) integrando ingestão + extração MediaPipe + features por frame + agregação por janela + relatório de qualidade | [src/mediapipe_seguranca/pipeline.py](../src/mediapipe_seguranca/pipeline.py), [main.py](../main.py) | Pipeline executável end-to-end com saídas validadas em [data/processed/frame_features_real.csv](../data/processed/frame_features_real.csv), [data/processed/window_features_real.csv](../data/processed/window_features_real.csv) e [data/processed/data_quality_report.json](../data/processed/data_quality_report.json) (Fase 4, 2026-04-25) |
